...
class EmployerController extends Controller
{
  ...
  public function index()
  {
    try {
      ...
      $users = User::with(['city', 'state'])->when($status, function ($users) use ($status) {
        if ($status != '-1') {
          $status = conditionalStatus($status);
          $users->where('status', '=', $status);
        }
      })->where('role_id', '=', 3)->orderBy('id', 'desc')->paginate(20);
      // dd( $users);
      return view('frontend.employers-list', compact('users'));
    } catch (\Exception $e) {
      dd($e);
