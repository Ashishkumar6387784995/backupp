...
class ShiftController extends Controller
{
    public function index()
    {
        try {
            ...
            return view('company.pages.shift.list', compact('page_title', 'page_description', 'breadcrumbs',  'jobs'));
        } catch (\Exception $e) {
            dd($e);
        ...
        try {
            if ($request->isMethod('post')) {
                ...
                try {
                    ...
                    DB::commit();
                    return response()->json(['success' => true, 'message' => 'Shift details added successfully!']);
                } catch (\Exception $e) {
                    DB::rollback();
                    return response()->json(['success' => false, 'message' => 'Failed to add Shift: ' . $e->getMessage()]);
                }
            }
            ...
            return view('company.pages.shift.add-new', compact('page_title', 'page_description', 'breadcrumbs', 'jobs'));
        } catch (\Exception $e) {
            dd($e);
            DB::rollback();
        ...
        try {
            if ($id) {
                ...
                if ($details) {
                    ...
                    return view('company.pages.shift.edit-new', compact('page_title', 'page_description', 'breadcrumbs', 'details'));
                } else {
                    return redirect()->back()->withErrors(['Shift details not found.']);
                }
            } else {
                return redirect()->back()->withErrors(['Shift details id is missing.']);
            }
        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->back()->withErrors($e->getMessage());
    ...
    public function pageSetting($action, $dataArray = [])
    {
        if ($action == 'edit') {
            ...
            $data['page_description'] = 'Add New Shift';
            $data['breadcrumbs'] = [
                [
                    'title' => 'Shift',
                    'url' => url('company/shift/list'),
                ]
            ];
