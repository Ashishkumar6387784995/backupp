    ...
    <div class="card-body">
        <div class="mb-7">
            <div class="row align-items-center">
                <form method="POST" action="{{ route('company.manageroleanduser.store') }}" class="w-100" enctype="multipart/form-data">
                    {{ csrf_field() }}
                                    ...
                                    <select name="department_id" id="" class="form-control select2" isrequired="required">
                                        ...
                                        @if ($departments)
                                            @foreach ($departments as $department)
                                                <option value="{{ $department->id }}">{{ $department->department->department_name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
