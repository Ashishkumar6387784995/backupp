...
@section('Shiftmaster','active menu-item-open')
@section('content')
<div class="col-xl-12">
   <div class="row">
      <div class="col-sm-12">
         <form id="multiStepForm" action="{{ route("company.job.update",["id"=>request("id")]) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="step row" id="step1">
               <div class="col-sm-7">
                  <div class="card">
                     <div class="card-body">
                        <h3 class="text-center">Step 1: Create Shift</h3>
                        <div class="mb-3 mt-6">
                        ...
                        <div class="mb-3">
                           ...
                           <input name="job_title" type="text" id="job_title" value="{{ $details->job_title }}" class="form-control" placeholder="Enter Job Title" isrequired="required">
                        </div>
                        <div class="mb-3">
                                 ...
                                 <div class="col">
                                    <select class="form-control select2" name="job_type" id="job_type" isrequired="required">
                                       ...
                                       @if (!empty($jobTypes))
                                           ...
                                           <option value="{{ $key }}" {{runTimeSelection($key,$details->job_type)}}>{{ $jtype }}</option>
                                           @endforeach
                                       @endif
                                    ...
                                    <select class="form-control select2" name="job_category" title="" id="job_category" isrequired="required">
                                       ...
                                       @if (!empty($categories))
                                          @foreach ($categories as $cate)
                                             <option value="{{ $cate->id }}"  {{runTimeSelection($cate->id,$details->job_category)}}>{{ $cate->category_name }}</option>
                                          @endforeach
                                       @endif
                                    </select>
                                 </div>
                        ...
                        <div class="mb-3">
                           ...
                           <textarea name="description" id="description" onkeyup="copyValue(this.value,'descriptionprev');" class="form-control" rows="3" isrequired="required">
                            {{ $details->description }}
                           </textarea>
                        </div>
                        <div class="row ">
                           <div class="col-md-12 mb-3">
                              ...
                              <input name="No_of_Workers" class="form-control" type="number" min="0" id="No_of_Workers" value="{{ $details->no_of_workers }}" onkeyup="copyValue(this.value,'No_of_Workers_prev','No_of_Workers_title');" placeholder="No. of Staff required" isrequired="required">
                           </div>
                           <div class="col-md-12 mb-3">
                              <label class="form-label">License &amp; Certification <span class="text-danger">*</span> </label>
                              <select class="form-control select2" id="license_required" name="license_required" isrequired="required">
                                 @php
                                     $licenseCertification = getJobLicenseCertifications();
                                 @endphp
                                 @if (!empty($licenseCertification))
                                     @foreach ($licenseCertification as $license)
                                       <option value="{{ $license->id }}" {{runTimeSelection($license->id,$details->license_required)}}>{{ $license->name }}</option>
                                     @endforeach
                                 @endif
                              </select>
                           </div>
                           ...
                           <div class="col-md-12 mb-3">
                              <label class="form-label">Experience Required for this Job <span class="text-danger">*</span></label>
                              <select class="form-control select2" id="experience" name="experience" isrequired="required">
                                 ...
                                 @if (!empty($job_experiences))
                                    @foreach ($job_experiences as $exp)
                                       <option value="{{ $exp->id }}" {{runTimeSelection($exp->id,$details->experience)}}>{{ $exp->name }}</option>
                                    @endforeach
                                 @endif
                        ...
                        <div class="row ">
                           <div class="col-md-12 mb-3">
                              <label class="form-label">Qualification Required for this Job <span class="text-danger">*</span></label>
                              <select class="form-control select2" id="qualification_required" name="qualification_required"  isrequired="required">
                                 @php
                                     $jobQualifications = getJobQualifications();
                                 @endphp
                                 @if (!empty($jobQualifications))
                                     @foreach ($jobQualifications as $qualification)
                                       <option value="{{ $qualification->id }}" {{runTimeSelection($qualification->id,$details->qualification_required)}}>{{ $qualification->name }}</option>
                                     @endforeach
                                 @endif
                              </select>
                           </div>
                           <div class="col-md-12 mb-3">
                              ...
                              <div>
                                 <select class="form-control select2" name="location" data-live-search="true" title="" id="location" isrequired="required">
                                    ...
                                    @if (!empty($getLocations))
                                       ...
                                       <option value="{{ $location->id }}"  {{runTimeSelection($location->id,$details->location)}}>{{ $location->location_name }}</option>
                                       @endforeach 
                                    @endif
                                 </select>
                              </div>
               ...
               <div class="col-sm-5">
                  <div class="card">
                     <div class="card-body">
                        <h3 class="text-center">Step to create new Shift</h3>
                        <ul class="step-list">
                           <li class="active"><strong>1.Create Shift</strong>Title, Job Type, Category ,Description and location</li>
                           ...
                           <div class="col-md-12 mb-6">
                              ...
                              <select class="form-control select2" id="pay_type_required" name="pay_type_required" isrequired="required">
                                 ...
                                 @if (!empty($getJobPayWages))
                                    ...
                                    <option value="{{ $paytype->name }}"  {{runTimeSelection($paytype->name,$details->pay_type_required)}}>
                                        {{$paytype->name }}                                                     
                                     </option>
                                    @endforeach 
                                 @endif
                           ...
                           <div class="col-md-6">
                              <div class="form-group mb-4 payment_week">
                                 ...
                                 <div class="input-group mb-6">
                                    ...
                                    <input min="0" type="number" class="form-control" name="price_from" id="price_from" value="{{ $details->price_from }}" onkeyup="copyValue(this.value,'price_from_prev');"  isrequired="required">
                                 </div>
                              </div>
                           </div>
                           <div class="col-md-6">
                              <div class="form-group mb-4 payment_week">
                                 ...
                                 <div class="input-group mb-6">
                                    <span class="input-group-text" id="basic-addon3">$</span>
                                    <input min="0" type="number" class="form-control" name="price_to" id="price_to" value="{{ $details->price_to }}" onkeyup="copyValue(this.value,'price_to_prev');" isrequired="required">
                                 </div>
                              </div>
                           </div>
                                    ...
                                    <div class="">
                                       <div class="row mb-3">
                                          ...
                                          <div class="employer_question_div  col-md-12 mb-3">
                                             ...
                                             <textarea name="employer_questions" id="employer_questions" onkeyup="copyValue(this.value,'employer_questions_prev');" class="form-control" rows="3" placeholder="Tell your Employer Question....">{{ $details->employer_questions }}</textarea>
                                          </div>
                                       </div>
                                       <div class="row">
                     ...
                     <div class="card-body">
                        ...
                        <div class="row mt-6 mt-6" id="contact_detail_info">
                           <div class="form-group col-md-6">
                              ...
                              <input type="text" class="form-control" id="contact_company_name" name="contact_company_name" value="{{ $details->contact_company_name }}" placeholder="Enter Company Name" isrequired="required">
                          </div>
                      
                          <div class="form-group col-md-6">
                              <label for="contact_email">Email Address</label>
                              <input type="email" value="{{ $details->contact_email }}" class="form-control" id="contact_email" name="contact_email" placeholder="Enter email address" isrequired="required">
                          </div>
                      
                          <div class="form-group col-md-6">
                              <label for="contact_phone">Phone Number</label>
                              <input type="tel" value="{{ $details->contact_phone }}" class="form-control" id="contact_phone" name="contact_phone" placeholder="Enter phone number" isrequired="required">
                          </div>
                           <div class="form-group col-md-6">
                              ...
                              <div>
                                 <select name="contact_country" id="contact_country" class="form-control select2" isrequired="required">
                                    ...
                                    @if (!empty($countries))
                                       ...
                                       <option value="{{$key}}" data-name="{{$country}}" {{runTimeSelection($key,$details->contact_country)}}>
                                          {{$country}}
                                       </option>
                                       @endforeach
                                    @endif
                           ...
                           <div class="form-group col-md-6">
                              ...
                              <input type="text" value="{{ $details->contact_zip }}" class="form-control" id="contact_zip" name="contact_zip" placeholder="Enter ZIP code" isrequired="required">
                           </div>
                           <div class="form-group col-md-6">
                              <label for="contact_website">Website URL</label>
                              <input type="url" value="{{ $details->contact_website }}" class="form-control" id="contact_website" name="contact_website" placeholder="Enter website URL">
                           </div>
                           <div class="form-group col-md-12">
                              <label for="contact_address">Address <span class="text-danger">*</span></label>
                              <textarea type="text" cols="3" class="form-control" id="contact_address" name="contact_address" placeholder="Enter address">{{ $details->contact_address }}</textarea>
                          </div>
                     ...
                     <div class="card-body">
                        ...
                        <div class="row mt-6" id="payment_info">
                           <div class="hourly_info_container role_payment_125 " id="hourly_info_container_125">
                              ...
                              <div class="form-check form-check-inline">
                                 <input class="form-check-input company_payment_status" type="radio" name="company_payment_status" id="company_payment_status" checked="" value="show">
                                 <label class="form-check-label" for="company_payment_status">Show</label>
                              </div>
                              <div class="form-check form-check-inline">
                                 <input class="form-check-input company_payment_status" type="radio" name="company_payment_status" id="company_payment_status" value="hide">
               ...
               <div class="col-sm-7">
                  <div class="card">
                     <div class="card-body">
                        <div class="row">
                           <div class="col-md-12">
                              <img class="preview_logo flex-shrink-0 img-fluid border rounded" src="{{ asset('uploads/jobs/logos/' . $details->logo) }}" alt="" id="preview" style="width: 80px; height: 80px;">
                           </div>
                           <div class="col-md-12">
                        ...
                        <div class="contact_info_com">
                           ...
                           <div class="col-md-12">
                              <h5 class="company-name" id="company-name-prev">Spyfoxx</h5>
                              <div class="">
                                 <div class="text-truncate mb-2">
                                    <i class="far fa-envelope me-2"></i>Email Id: <span id="email_prev">wali@backupp.com.au</span> 
                                 </div>
                                 <div class="text-truncate mb-2">
                                    <i class="fas fa-phone-volume"></i>
                                    Phone: <span id="mobile_prev">  3453453</span> 
                                 </div>
                              </div>
                     ...
                     <div class="card-body">
                        ...
                        <div class="mb-3">
                           <div class="row mb-3">
                              ...
                              <div class="col mb-6">
                                 <input type="submit" value="Publish" class="btn btn-primary d-block">
                              </div>
                           </div>
...
<script type="text/javascript">
    ...
    var countryId = '@if(isset($details->contact_country)){{$details->contact_country}}@endif';
    var stateId = '@if(isset($details->contact_state)){{$details->contact_state}}@endif';
    var cityId = '@if(isset($details->contact_city)){{$details->contact_city}}@endif';
        ...
        $.ajax({
            ...
            success: function(states) {
                $.each(states, function(key, state) {
                    ...
                    $('#contact_state').append('<option value="'+ state.id +'" '+ selected +'>'+ state.name +'</option>');
                });

                if (selectedState) {
                    $('#contact_state').val(selectedState).trigger('change');
                }
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function(cities) {
                $.each(cities, function(key, city) {
                    ...
                    $('#contact_city').append('<option value="'+ city.id +'" '+ selected +'>'+ city.name +'</option>');
                });

                if (selectedCity) {
                    $('#contact_city').val(selectedCity).trigger('change');
                }
            }
        });
    }
...
async function nextStep(step) {
    ...
    if (step === 5) {
        ...
        if (!contact_company_name) {
            ...
            showInputFieldError($('#contact_country'), 'Country are required');  
        } else if(!contact_state){
            showInputFieldError($('#contact_state'), 'State are required');  
        } else if(!contact_city){
            showInputFieldError($('#contact_city'), 'City are required');  
        } else if (!contact_zip) {
            showInputFieldError($('#contact_zip'), 'Zip code are required');
        } else if(!contact_address){
            showInputFieldError($('#contact_address'), 'Address are required')
        } else {
            showStep(step);
        }
    }
...
const handleFormSubmit = async () => {
   let formData = new FormData();
    ...
    $.ajax({
        url: '{{ route("company.job.update",["id"=>request("id")]) }}', 
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
