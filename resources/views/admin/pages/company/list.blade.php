        ...
        <table class="table table-bordered table-hover" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{url('admin/company/user-verify/'.$value->user_id.'/'.$value->user->is_verified)}}" data-url="" onclick="return confirm('Do you want to verify?');"> <span class="label label-lg font-weight-bold label-light-{{($value->user->is_verified == 1) ? 'success' : 'danger'}} label-inline">{{($value->user->is_verified == 1) ? 'Verified' : 'Pending'}}</span></a>
                    </td>
                    <td>
                        <a href="{{url('admin/company/update-status/'.$value->user_id.'/'.$value->user->is_active)}}"  onclick="return confirm('Do you want to change status?');"> <span class="label label-lg font-weight-bold label-light-{{($value->user->is_active == 1) ? 'success' : 'danger'}} label-inline">{{($value->user->is_active == 1) ? 'Active' : 'InActive'}}</span></a>
                    </td>
                    <td>{{$value->created_at }}</td>  
                    <td>
                        <a href="{{url('/admin/company/edit/'.$value->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit details" data-toggle="tooltip">
