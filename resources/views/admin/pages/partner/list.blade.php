...
<div class="card card-custom">
    <div class="card-header flex-wrap border-0 pt-3 pb-0">
        <div class="card-title">
            <h3 class="card-label">Partners List
                <!-- <div class="text-muted pt-2 font-size-sm">Datatable initialized from HTML table</div> -->
            </h3>
        </div>
        <div class="card-toolbar"> 
            <!--begin::Button-->
            <a href="{{url('/admin/partner/add-user')}}" class="btn btn-primary font-weight-bolder">
                <i class="la la-plus"></i>Add Partner</a>
            <!--end::Button-->
        </div>
        ...
        <table class="table table-bordered table-hover" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="javascript:void(0)" data-url="{{url('admin/partner/update-status/'.$value->id.'/'.$value->status)}}" onclick="changeStatus(this)"> <span class="label label-lg font-weight-bold label-light-{{($value->status == 1) ? 'success' : 'danger'}} label-inline">{{($value->status == 1) ? 'Active' : 'InActive'}}</span></a>
                    </td>
                    <td>{{$value->created_at }}</td>  
                    <td>
                        <a href="{{url('/admin/partner/edit/'.$value->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit details" data-toggle="tooltip">
