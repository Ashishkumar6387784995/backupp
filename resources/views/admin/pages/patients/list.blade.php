...
<div class="card card-custom">
    <div class="card-header flex-wrap border-0 pt-3 pb-0">
        ...
        <div class="card-toolbar">
            ...
            <!--begin::Button-->
            <a href="{{url('/admin/patients/add/'.$customerId)}}" class="btn btn-primary font-weight-bolder">
                <i class="la la-plus"></i>Add a New Patient</a>
            <!--end::Button-->
        </div>
            ...
            <div class="row col-lg-12 pl-0 pr-0">
                ...
                <!-- <div class="col-sm-5">
                    <div class="dataTables_length">
                        ...
                        <a href="{{url('/admin/patients/list')}}" class="btn btn-default mt-7" data-toggle="tooltip" title="Reset">Reset</a>
                    </div>
                </div>
            </div> -->
        </form>
        ...
        <table class="table table-bordered table-hover" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $value->gender	 }}</td>
                    <td>{{$value->dob?  date('d-m-Y',strtotime($value->dob))	: 'NA' }}</td>
                    <td> {{($value->relation == 1) ? 'Self' : 'Other'}} </td>
                    @if(!$customerId)
                    <td>
                        <a href="{{url('admin/customers/edit/'.$value->customer->id)}}">
                            {{ $value->customer->first_name .' ' . $value->customer->last_name }}
                        </a>
                    </td>
                    @endif
                    <!-- <td>
                        <a href="javascript:void(0)" data-url="{{url('admin/patients/update-status/'.$value->id.'/'.$value->status)}}" onclick="changeStatus(this)">
                    ...
                    <td>
                        <a href="{{url('/admin/patients/edit/'.$value->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit details" data-toggle="tooltip">
