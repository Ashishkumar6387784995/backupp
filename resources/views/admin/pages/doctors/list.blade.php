...
<div class="card card-custom">
    <div class="card-header flex-wrap border-0 pt-3 pb-0">
        <div class="card-title">
            <h3 class="card-label">Doctors List
            ...
            <div class="row col-lg-12 pl-0 pr-0">
                ...
                <div class="col-sm-3">
                    <div class="dataTables_length">
                        ...
                        <a href="{{url('/')}}/admin/doctors/list" class="btn btn-default mt-7" data-toggle="tooltip" title="" data-original-title="Reset">Reset</a>
                    </div>
                </div>
            </div>
        </form>
        ...
        <table class="table table-bordered table-hover" id="myTable">
            ...
            <tbody> 
                ...
                <tr>
                    ...
                    <td>{{$value->name }}</td>
                    <td>{{$value->doctor_code }}</td>
                    <td>{{$value->mobile }}</td>
                    <td>{{$value->department ? $value->department->department_name : '' }}</td>
                    <td>
                        <a href="javascript:void(0)" data-url="{{url('admin/doctors/update-status/'.$value->id.'/'.$value->status)}}" onclick="changeStatus(this)"> <span class="label label-lg font-weight-bold label-light-{{($value->status == 1) ? 'success' : 'danger'}} label-inline">{{($value->status == 1) ? 'Active' : 'InActive'}}</span></a>
                    </td>
                    <td>
                        <a href="{{url('/admin/doctors/edit/'.$value->id)}}" class="btn btn-sm btn-clean btn-icon" title="Edit details" data-toggle="tooltip">
                            <i class="la la-edit"></i>
                        </a>
                        <a href="javascript:void(0)" data-url="{{url('/admin/doctors/delete/'.$value->id)}}" class="btn btn-sm btn-clean btn-icon" data-toggle="tooltip" title="Delete" onclick="deleteItem(this)">
